...
public class NetHeaders {
    ...
    public static Map getHeadMap() {
        Serializable cookieData = SharedPrefrenceUtils.getObject(BaseApplication.getInstance(), "cookieData");
        Map<String, String> headers = new HashMap<>();
        headers.put("Cookie", Token.getCookie());
        headers.put("Pragma", "no-cache");
        headers.put("Cache-Control", "no-cache");
//        headers.put("Authorization", Application10.getApplication().mToken);
        headers.put("UUID", BaseApplication.getInstance().mUuid.toString());
